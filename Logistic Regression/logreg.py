...
class LogReg:
    ...
    def sg_update(self, train_example, iteration, use_tfidf=False):
        ...
        for ex in train_example.x:
            if ex!=0:
                self.w[i]=self.w[i]+ self.eta*((train_example.y-sigmoid(ex.dot(self.w)))-2*self.lam*self.w[i])

            i+=1

        wtx= train_example.x.dot(self.w)
        ...
        if(iteration==0):
            ...
            for d in range(0,size_w+1):
                self.last_update[d]=0
            

        wtx= train_example.x.dot(self.w) 
        sig= sigmoid(wtx)
        gradient= (train_example.y-sig)*self.eta(iteration)
        shrinkage=(1-2*self.lam*self.eta(iteration))
        i=0
        for ex in train_example.x:
            if ex!=0:
                
                self.w[i]+=gradient*ex
                self.last_update[i]=(iteration-self.last_update[i]+1)

            i+=1
        
        j=0
        for tex in train_example.x:
            if(tex!=0):
                if(j!=0):
                    lazysparse=pow(shrinkage,self.last_update[j])
                    self.w[j]=self.w[j]*lazysparse
            j+=1               
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--positive", help="Positive class",
                           type=str, default="../data/hockey_baseball/positive.txt", required=False)
    argparser.add_argument("--negative", help="Negative class",
                           type=str, default="../data/hockey_baseball/negative.txt", required=False)
    argparser.add_argument("--vocab", help="Vocabulary that can be features",
                           type=str, default="../data/hockey_baseball/vocab.txt", required=False)
