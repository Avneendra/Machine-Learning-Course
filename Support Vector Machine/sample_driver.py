...
if __name__ == "__main__":
	...
	data = ThreesAndEights("../data/mnist.pkl.gz")
        clf=SVC(kernel='rbf', C=100.0, gamma=1)
        print "Data X train"
        print data.x_train
        print "Data Y train"
        print data.y_train
        
        clf=clf.fit(data.x_valid[:args.limit], data.y_valid[:args.limit])
	...
	# -----------------------------------
        ...
        print "CLF ACCURACY"
        print clf.score(data.x_train[:args.limit],data.y_train[:args.limit])
